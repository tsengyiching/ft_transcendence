...
import { Ball, Paddle, Player, ObjectToCollide, Match , Pos} from './pong.interface';
...
export class PongService {
  ...
  private afterGoalUpdate(matchId:number, scored: boolean) {
	...
	UpdateGame(gameId:number) { // voir si on peut faire une boucle unique plutot que 2 boucle a chaque fois
		this.matches.forEach(match => {
			if (match.id === gameId) {
			const touch = ballCollisionToPaddle(match.ball, match.paddleL, match.paddleR);
			if (touch.x || touch.y) {
			  ...
			  match.lastp = false;
			  const toWall = ballCollisiontoWall(match.ball, {
				tl: { x: 0, y: 0 },
				tr: { x: W, y: 0 },
				bl: { x: 0, y: H },
				br: { x: W, y: H },
